...
public:
   ...
   void              ProcessResponse(string response)
   {
      ...
      for(int i = 0; i < ArraySize(parts); i++) {
         ...
         if(json.Deserialize(parts[i])) {
            ...
            else if(msgType == "study_data") {
               // Study data
               TriggerEvent(m_loginHandlers, parts[i]);
            }
            else if(msgType == "protocol_error") {
               // Error
               TriggerEvent(m_errorHandlers, "Protocol error: " + json["p"].ToStr());
            }
            else if(msgType == "set_auth_token") {
               // Authentication response
               m_isLogged = true;
               TriggerEvent(m_loginHandlers, parts[i]);
            }
            else if(msgType == "quote_add_symbols") {
            ...
            else if(msgType == "quote_remove_symbols") {
            ...
            else if(msgType == "update_study_inputs") {
            ...
            else if(msgType == "get_session_info") {
            ...
            } else if(StringFind(parts[i], "~m~") >= 0) {
      ...
      while(GetTickCount() - startTime < 10000) { // Increased timeout for authentication
         ...
         if(ReceiveResponse(response, 1000)) {
            ...
            if(resp.Deserialize(response)) {
               ...
               else if(resp["m"].ToStr() == "session_info") {
                  if(resp["p"].Size() > 0) {
                     CJAVal params = resp["p"];
                     if(params["sid"].ToStr() != "") {
                        m_sessionId = params["sid"].ToStr();
   ...
   void              OnTimer()
   {
      if(!m_isConnected || m_socket == INVALID_HANDLE) return;
...
public:
   ...
   bool              Update()
   {
      if(m_api == NULL || !m_api.IsLogged()) return false;
   ...
   {
      if(StringFind(data, "\"m\":\"qsd\"") < 0) return;
...
public:
   ...
   void              ProcessUpdate(string data)
   {
      if(StringFind(data, "\"m\":\"cs\"") < 0) return;
...
private:
   ...
   {
      switch(timeframe) {
         case TIMEFRAME_TV_1:  return "1";
         case TIMEFRAME_TV_5:  return "5";
         case TIMEFRAME_TV_15: return "15";
         case TIMEFRAME_TV_30: return "30";
         case TIMEFRAME_TV_60: return "60";
         case TIMEFRAME_TV_240: return "240";
         case TIMEFRAME_TV_D:  return "D";
         case TIMEFRAME_TV_W:  return "W";
         case TIMEFRAME_TV_M:  return "M";
         default: return "60";
      }
   }
};
...
public:
   ...
   void              ProcessUpdate(string data)
   {
      if(StringFind(data, "\"m\":\"study_data\"") < 0) return;
